...
export default function Admin() {
  ...
  const fetcher = () => axios('/api/listado-ordenes-agosto').then(datos => datos.data)
  const { data, error} = useSWR('/api/listado-ordenes-agosto',fetcher,{refreshInterval: 100} )
  ...
  useEffect(() => {
    ...
    try {
      let url = '/api/listado-ordenes-agosto';
  ...
  return(
    <LayoutInicioAdminGeneral pagina={'Listado-OC'}>
      <h1 className="text-gray-500 text-lg font-semibold pb-8">Informe De Compras Por Obra Agosto 2023</h1>
      <div className='mt-auto hidden'>
        ...
        <Link href="/calendario" className="w-3/4 m-auto border border-indigo-500 bg-indigo-500 text-white rounded-md px-4 py-1 transition duration-500 ease select-none hover:bg-indigo-600 focus:outline-none focus:shadow-outline" onClick={calcularVolumen}>Meses Anteriores</Link>
      </div>
          </div>
        </div>
