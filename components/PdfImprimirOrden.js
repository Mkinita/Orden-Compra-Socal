...
const ListadoPdfOcGeneral = ({ocpedidos}) => {
    const {id,pedido01, fecha,} = ocpedidos
    ...
    const completarOc = async () => {
        ...
        try {
           ...
           await axios.post(`/api/ocgeneraladmin/${id}`)
            toast.success('🏠')
            setTimeout(() =>{
              router.push('/listado-oc-general-ocgeneral')
          },1000)
        } catch (error) {
            toast.error('Hubo un error')
        }
  ...
  return (
    ...
    <>
    <div class="py-5 flex w-full items-center justify-center bg-white bg-cover bg-no-repeat">
        <div class="rounded-xl bg-gray-200 bg-opacity-50 px-16 py-5 shadow-lg backdrop-blur-md max-sm:px-8">
            ...
            <div class="mb-8 flex flex-col items-center">
                ...
                <div className="grid grid-cols-2 gap-2 font-bold py-2">
                <div className="text-center rounded-xl  bg-lime-600 hover:bg-lime-700 hover:text-white text-black p-1  shadow-xl backdrop-blur-md transition-colors duration-300">
                        <button type="button" onClick={completarOc}>
                            Lista
                        </button>
                    </div>
                    <div className="text-center rounded-xl bg-red-600 hover:bg-red-700 hover:text-white text-black p-1  shadow-xl backdrop-blur-md transition-colors duration-300">
                        <button type="button" onClick={anularordendecompra}>
                            Anular
                        </button>
                    </div>
