...
const ModalPedido01 = () => {
  const { proveedor, handleChangeModal, handleAgregarPedido01, pedido01 } = useCombustible();
  const [cantidad, setCantidad] = useState(1);
  const [edicion, setEdicion] = useState(false);

  useEffect(() => {
    if (pedido01.some((ordenState) => ordenState.id === proveedor.id)) {
      const equipoEdicion = pedido01.find(
    ...
    <div className="md:flex gap-10">
      ...
      <div className="text-center">
        ...
        <button
          ...
          onClick={() => handleAgregarPedido01({ ...proveedor})}
        >
