...
const ModalProveedor = () => {
  ...
  useEffect(() => {
    if (orden.some((ordenState) => ordenState.id === ocpedidos.id)) {
      const equipoEdicion = orden.find(
        (ordenState) => ordenState.id === ocpedidos.id
      );
      setEdicion(true);
    }
  }, [ocpedidos, orden]);

  const {id, nombre,  folio, emisor, obra,
            ...
            <div className='py-1 border-b last-of-type:border-0 items-center'>
                ...
                <header>
                    ...
                    <div className="space-y-1">
                        ...
                        <p className="text-xs font-bold">Nombre: {orden.nombre} {nombre01}</p>
                        <p className="text-xs font-bold">Rut: {ocpedidos.rut} {rut01}</p>
                        <p className="text-xs font-bold">-</p>
                        <p className="text-xs font-bold">{ocpedidos.chofer}</p>
